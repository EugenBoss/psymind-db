...
begin
  ...
  if v_ignore_reason is not null then
    ...
    -- Practice Pilot access changes are part of this same transaction. If any
    -- mutation fails, neither the entitlement nor the Stripe event becomes
    -- terminal, so the webhook can mark the claim retryable and replay safely.
    if (v_current_is_practice_pilot or v_target_is_practice_pilot)
      ...
      update public.api_organization_members m
      ...
      where exists (
        select 1
        from public.api_organization_members m
      ...
      where m.user_id = p_user_id
        ...
        and exists (
          select 1 from public.api_organizations o
      ...
      if p_transition = 'checkout' then
        ...
        returning id into v_pilot_reservation_id;
